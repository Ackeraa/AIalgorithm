...
def _generate_parent(length, geneSet, get_fitness):
    ...
    return Chromosome(genes, fitness, Startegies.Create)
...
def _mutate(parent, geneSet, get_fitness):
    ...
    return Chromosome(genes, fitness, Strategies.Mutate)
...
def _crossover(parentGenes, index, parents, get_fitness, crossover, mutate, generate_parent):
    ...
    childGeens = crossover(parentGenes, parents[donorIndex].Genes)
    if childGenes is None:
        parents[donorIndex] = generate_parent()
        return mutate(parents[index])
    fitness = get_fitness(childGenes)
    return Chromosome(genes, fitness, Strategies.Crossover)

...
if __name__ == '__main__':
    ...
    r = 3
    arrays = [1, 2, 100, 10]
    board = Board(arrays, l, r, 0)
    state = GameState(next_to_move = 1, board = board)
    node = MonteCarloNode(state)
    monteCarlo = MonteCarloTreeSearch(node)
    
    best = monteCarlo.best_action(simulations_number)
    monteCarlo.root.parent = monteCarlo.root
    build_tree(monteCarlo.root)
    print("FUCL", len(best.children))
